...
class ActivityLedgerTests(unittest.TestCase):
    ...
    def test_fastapi_repo_truth_routes_are_not_registered_after_go_cutover(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            service, workspace_id = self._create_service(tmp_dir)
            ...
            with patch.object(app_main, "SERVICE", service):
                with TestClient(app_main.app) as client:
                    for endpoint in [
                        ...
                        response = client.get(f"/api/workspaces/{workspace_id}/{endpoint}")
                        self.assertEqual(response.status_code, 404, msg=endpoint)
