...
- Python compatibility CLI and FastAPI runtime discovery/probe paths now delegate to Go `xmustard-ops runtime`, so Python no longer owns runtime listing, model listing, local-agent capabilities, or bounded runtime probe semantics for those compatibility surfaces.
...
- The remaining runtime/session cutline is exact: Go still owns long-lived issue/workspace-query process launch, live log writing, PID tracking, retry/cancel, and run summary persistence; Python compatibility still carries the old runtime/session and terminal server plane for the non-migrated shell; Rust needs a managed-run/session contract before that can move without regressing cancellation and streamed run-log behavior.
